// ohne inline: 424178, mit inline 424150 (!!!)
inline void drawLineSafe(Arduino_Canvas *canvas, Point2D p1, Point2D p2, uint16_t color /**/) {
...
void drawJellyfish(Arduino_Canvas *canvas, Point2D bell[NUM_BELL_VERTICES],
  ...
  if (!wireframe) {
    ...
    for (int i = 0; i < BELL_POINTS_PER_RING; i++) {
      ...
      for (int r = 0; r < BELL_RINGS; r++) {
        ...
        int n_idx = 1 + r * BELL_POINTS_PER_RING + (i + 1) % BELL_POINTS_PER_RING;
  ...
  for (int j = 0; j < NUM_TENTACLES; j++) {
    int bi = 1 + (BELL_RINGS - 1) * BELL_POINTS_PER_RING + (j * (BELL_POINTS_PER_RING / NUM_TENTACLES));
