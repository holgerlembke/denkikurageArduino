...
Point2D project(Point3D p, float global_x, float global_y, float global_z /**/) {
  ...
  if (world_z < 40.0f) {  // 20.0
    p2.valid = false;
    return p2;
  }
  float scale = fov / world_z * scalefactor;
  p2.x = (int)(SCREEN_WIDTH / 2 + (p.x + global_x) * scale + 0.5);
  p2.y = (int)(SCREEN_HEIGHT / 2 + (p.y + global_y) * scale + 0.5);
  p2.z = world_z;  // Store depth
  p2.valid = (p2.x >= -50 && p2.x < SCREEN_WIDTH + 50 && p2.y >= -50 && p2.y < SCREEN_HEIGHT + 50);
  return p2;
}
